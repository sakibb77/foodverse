...
const RecipeItem = ({ favouriteHandler, savedItems }) => {
  ...
  useEffect(() => {
    if (!recipe) return;
    setItemSetStatus(savedItems?.some());
  }, []);
        ...
        <div className="recipe-left flex flex-col gap-5">
          <div className="flex justify-center md:justify-between items-center flex-col md:flex-row">
            ...
            <button
              ...
              className="bg-gradient-to-br from-pink-400 to-pink-600 p-3 px-8 text-pink-50 rounded-lg mt-2 \ tracking-wider uppercase text-sm font-medium shadow-md shadow-pink-200 hover:shadow-lg hover:shadow-pink-300 duration-300"
            >
              + save as favourite
            </button>
          </div>
